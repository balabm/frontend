  ...
  Future<void> _cropImage() async {
    if (imagePath != null) {
      CroppedFile? croppedFile = await ImageCropper().cropImage(
        ...
        compressQuality: 60, // Reduced quality to keep file size small
        uiSettings: [
          AndroidUiSettings(
            toolbarTitle: 'Crop',
    ...
    if (imagePath != null) {
      ...
      try {
        ...
        if (response.statusCode == 200) {
          ...
          _showErrorToast(
              context, decodedResponse['message'] ?? 'Error processing image');
        } else {
        ...
        _showErrorToast(
            context, 'Error sending image. Please check the settings.');
      } finally {
        if (mounted) {
          setState(() {
            _isLoading = false;
    ...
    Widget toast = Container(
      ...
      decoration: BoxDecoration(
        ...
        borderRadius: BorderRadius.circular(
            4.0), // Reduced border radius for a sharper look
        boxShadow: const [
          BoxShadow(
                        ...
                        child: ClipRRect(
                          ...
                          child: imagePath != null
                              ? Image.file(
                                  File(imagePath!),
                                  key:
                                      ValueKey('${imagePath}_${_forceRebuild}'),
                                  fit: BoxFit.contain,
